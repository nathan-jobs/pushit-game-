...
__all__ = ['ignore_exception', 'auto_update', 'cree_fenetre', 'ferme_fenetre',
           'mise_a_jour', 'ligne', 'fleche', 'polygone', 'rectangle', 'cercle',
           'point', 'marque', 'image', 'texte', 'longueur_texte',
           'hauteur_texte', 'efface_tout', 'efface', 'efface_marque',
           'attente_clic', 'attente_deplacement', 'attente_touche',
...
def polygone(points, couleur='black', remplissage='', epaisseur=1, tag=''):
    ...
    :param str couleur: couleur de trait (défaut 'black')
    :param str remplissage: couleur de remplissage (défaut '')
    :param float epaisseur: épaisseur de trait en pixels (défaut 1)
    :param str tag: étiquette d'objet (défaut : pas d'étiquette)
    :return: identificateur d'objet
    """
...
def attente_deplacement():
    """Attend que l'utilisateur déplace la souris et renvoie un triplet ``( 
    x, y)``, où ``x`` et ``y`` sont l'abscisse et l'ordonnée du pointeur. 
...
def attente_clic_ou_touche():
    ...
    :return: ``(x, y, 'ClicDroit')``, ``(x, y, 'ClicGauche')``, ``(x,
    y, 'Deplacement')`` ou ``(-1, val,\ 'Touche')``

    """
    while True:
        ev = donne_evenement()
        type_ev = type_evenement(ev)
        if "Clic" in type_ev or type_ev == 'Deplacement':
            return clic_x(ev), clic_y(ev), type_ev
        elif type_ev == "Touche":
